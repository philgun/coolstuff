...
//
  ...
  parameter SI.Power P_dummy = 100e6;
  parameter SI.Power P_net(fixed=false);
  parameter SI.Temperature T_in_ref_blk = 1127.4;
  parameter SI.AbsolutePressure p_high = 24126422.37 "high pressure of the cycle";
  parameter Real PR = 2.99 "Pressure ratio";
  parameter SI.Temperature T_cold_set = 819.24;
  parameter SI.Temperature delta = 93.67 "T_in_ref_blk - T_high";
  
  // *********************PB Params - Changeable Operation Param
  parameter Real load = 0.69;
  parameter SI.Temperature T_HTF_in = 1017.59;
  parameter SI.Temperature T_amb_input = 308.41;
...
//st_simulate --np=0 --stop 1 --step 0.002 --integOrder 5 --tolerance 1e-06 --solver dassl --nls homotopy sCO2PBCalculator.mo P_net=67690794.6 T_in_ref_blk=1098.81 p_high=24315878.42 PR=2.9 T_cold_set=828.61 delta=77.53 load=0.71 T_HTF_in=1006.91 T_amb_input=311.84
