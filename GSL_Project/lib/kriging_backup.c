...
void* initSurrogate(double P_net, double T_in_ref_blk, double p_high, double PR, double pinch_PHX, double dTemp_HTF_PHX, double load_base, double T_amb_base, double eta_gross_base, double eta_Q_base, int user_choice, char* base_path)
{   
    char line[2000];

    char* test_algorithm = "/test_data/test_algorithm.csv";
    int lenlen = strlen(base_path) + strlen(test_algorithm) + 1;
    char* filepath_test_algorithm = malloc(sizeof(char*)*lenlen);
    strcpy(filepath_test_algorithm,base_path);
    strcat(filepath_test_algorithm,test_algorithm);
    ...
    */
    ...
    char* filepathtraining;
    
    if(status == 1)
    {
        filepathtraining = validationSurrogate(P_net, T_in_ref_blk, p_high, PR, pinch_PHX, dTemp_HTF_PHX, load_base, T_amb_base, eta_gross_base, eta_Q_base, user_choice, base_path,match_index);
    }

    int rows = getNumOfData(filepathtraining);

    //if kriging
    Kriging_training_struct* Kriging_training_point = malloc(sizeof(Kriging_training_struct)*rows);/*Declare a variable to store the string from CSV*/
    load_Kriging_training_data(filepathtraining,Kriging_training_point);
...
{   
    ...
    load_Kriging_training_data(filepath,Kriging_training_point);
...
void* load_Kriging_training_data(char* filepathtraining, Kriging_training_struct *Kriging_training_point)
{
    int rows = getNumOfData(filepathtraining);
    char line[limitSize];
    /*Open the file*/
...
char* validationSurrogate(double P_net, double T_in_ref_blk,double p_high, double PR, double pinch_PHX, double dTemp_HTF_PHX,double load_base, double T_amb_base, double eta_gross_base, double eta_Q_base,int user_choice, char* base_path, int match_index)
{
    char line[limitSize];
